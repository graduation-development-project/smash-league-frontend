const images = {
  heroImage: require("@/assets/images/Hero.png"),
  heroBadmintonImage: require("@/assets/images/Hero-Badminton.png"),
  featuredTourCardMain: require("@/assets/images/TourCardMain.png"),
  featuredTourCard: require("@/assets/images/TourCard.png"),
  sponsorImage: require("@/assets/images/Sponsor.png"),
  viewImage: require("@/assets/images/view.png"),
  refereesImage: require("@/assets/images/Referees.jpg"),
  googleImage: require("@/assets/images/google.png"),
  orangeBackgroundImage: require("@/assets/images/orange-badminton-image.jpg"),
  greenBackgroundImage: require("@/assets/images/green-badminton-image.jpg"),
  backgroundImage: require("@/assets/images/background-image.png"),
  badmintonDoubleImage: require("@/assets/images/badminton-doubles.jpg"),
  badmintonChampionShip: require("@/assets/images/badminton-championship.jpg"),
  badmintonSummerOlympics: require("@/assets/images/badminton-summer-olympics.jpg"),
  badmintonRacket: require("@/assets/images/badminton-racket.webp"),
  avatarMale: require("@/assets/images/avatar-male.png"),
  logoSL: require("@/assets/images/logoSL.png"),
};

export default images;
