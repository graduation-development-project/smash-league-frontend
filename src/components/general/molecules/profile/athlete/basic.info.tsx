      ...
      <div className="w-full h-full flex justify-between">
        <div className=" w-full h-full flex flex-col gap-3">
          <div className="w-full h-full flex gap-2">
            <InfoBox title="AGE" info={calculateAge(info?.dateOfBirth).toString() || "No Info"} />
            <InfoBox title="HEIGHT(cm)" info={formatHeight(info?.height) || "No Info"} />
        ...
        <div className="w-full h-full flex flex-col px-9">
          ...
          />
          <Divider />
          <InfoLine quest="Date Of Birth" ans={formatDate(info?.dateOfBirth) === "Invalid Date" ? "No Info" : formatDate(info?.dateOfBirth)} />
          <Divider />
