...
import {
  ...
  RegisterAthleteTournamentSubmitFormProps,
} from '@/types/types';
...
const RegisterTeamTourForm = ({
  ...
  const [athleteFiles, setAthleteFiles] = useState<
    Record<number, Record<string, File[]>>
  >({});
  const [dataStep1, setDataStep1] = useState<any>(null);
  const [registerAthleteList, setRegisterAthleteList] = useState<
    RegisterAthleteTournamentFormProps[]
  >([]);
  ...
  const fetchRegisterSubmit = async () => {
    try {
      // setIsLoading(true);
      const response = await registerTournamentByTeamAPI(
        submitList,
        user.access_token,
      );
  ...
  const handleFinish = async () => {
    ...
    try {
      for (const athlete of beforeSubmit) {
        ...
        for (const [docType, files] of Object.entries(
          athlete.registrationDocumentCreator,
        )) {
        ...
        if (
          athlete.partnerId &&
          athlete.partnerId !== '' &&
          athlete.registrationDocumentPartner
        ) {
          for (const [docType, files] of Object.entries(
            athlete.registrationDocumentPartner,
          )) {
        ...
        submitList.push({
          ...
          registrationDocumentPartner: athlete.partnerId
        ...
        >
          ...
          <div className="w-full flex justify-center items-center gap-5 font-semibold text-primaryColor">
            ...
            {currentStep === steps.length - 1 && (
              ...
              >
                Done {isLoading && <LoadingOutlined />}
              </Button>
            )}
            {currentStep < steps.length - 1 && (
              <Button
