...
import {
  createSponsorsDetailsTourAPI,
  deleteSponsorAPI,
  getSponsorTourDetailsAPI,
} from '@/services/update-tour';
import {
  InfoCircleOutlined,
  LoadingOutlined,
  PlusCircleOutlined,
...
const UpdateSponsorsDetailsTour = ({
  ...
  const mainColor = detail?.mainColor;
  ...
  return (
    <div>
      ...
      <div className="p-4 rounded-md border">
        ...
        >
          <h3 className="px-4 py-2 rounded-md text-lg font-bold mb-4 bg-[#ffeee6] border-[#ff8243] text-primaryColor ">
            Add new sponsor
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            >
              <Input maxLength={10} placeholder="Sponsor's Name" />
            </Form.Item>
            <Form.Item
              label={'Logo'}
              name={'logo'}
              ...
              <div className="flex items-end">
                ...
                {isLimitReached(selectedType) && (
                  <Tooltip
                    ...
                    title={'Maximum limit reached for this type'}
                  >
                    <Button
      ...
      {/* Sponsor categories */}
      <div className="">
        {sponsorTypes.map((type: any, index: number) => (
          ...
          //                     <EmptyCard
          //                         description='No sponsors yet'
          //                         image='https://i.fbcd.co/products/resized/resized-750-500/1c6f55fb1e9a3eb4f7c93f4b659c11b2489de85652758322b4cb16cbf9d611d7.jpg'
          //                     />
          //                 </div>
          //             )}
          //         </div>
          //     </div>
          // </div>
          <div key={index} className="overflow-x-auto w-full pb-2">
            <div
              className={`
            flex gap-4 w-full
            ${['bronze', 'other'].includes(type.id) ? '' : 'flex-wrap'}
        `}
            >
              {sponsorList[type.id as keyof typeof sponsors]?.length > 0 ? (
                sponsors[type.id as keyof typeof sponsors]?.map(
                  (sponsor: any) => (
                    <Card key={sponsor.id} className="w-[200px] flex-shrink-0">
                      <div className="p-4">
                        <div className="flex flex-col items-center">
                          ...
                          <Button
                            onClick={() =>
                              fetchDeleteSponsor(detail?.id, sponsor.id)
                            }
                            className="w-full"
                          >
                            <Trash2 className="w-4 h-4 mr-2" /> Delete
