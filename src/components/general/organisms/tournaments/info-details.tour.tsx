...
import { formatDate, formatDateTime, formatMoney, formatOccurDate, formatTime } from '@/utils/format';
    ...
    return (
        <div className='w-full h-max flex flex-col gap-5 p-3'>
            ...
            <div className={`${styleCard} flex-col gap-4 text-base `}>
                ...
                <div className='flex gap-5'>
                    <ul className='flex flex-col gap-2 font-semibold'>
                        ...
                        <li className='flex gap-2 items-center'><AiOutlineFieldNumber color={`${color}`} size={21} />Number of events each person can register</li>
                        <li className='flex gap-2 items-center'><HiOutlineUser color={`${color}`} size={19} />Registration Fee for Individual</li>
                        <li className='flex gap-2 items-center'><HiOutlineUsers color={`${color}`} size={19} />Registration Fee for Pair</li>
                        <li className='flex gap-2 items-center'><MdReportGmailerrorred color={`${color}`} size={20} />Protest Fee</li>
                        <li className='flex gap-2 items-center'><GrAttachment color={`${color}`} size={17} />Required Attachments</li>
                    ...
                    <ul className='flex flex-col gap-2'>
                        ...
                        <li>{tour?.requiredAttachments?.map((attach: string) => (`${ATTACHMENTS_ENUM[attach as keyof typeof ATTACHMENTS_ENUM]} `))}</li>
                    </ul>
                </div>
            ...
            <div className={`${styleCard} flex-col gap-4 text-base `}>
                ...
                <div className='flex gap-5'>
                    <ul className='flex flex-col gap-2 font-semibold'>
                        <li className='flex gap-2 items-center'><FaRegCalendar color={`${color}`} size={18} />Draw Date</li>
                        <li className='flex gap-2 items-center'><FaRegClock color={`${color}`} size={18} />Check in Before</li>
                        <li className='flex gap-2 items-center'><FaRegCalendar color={`${color}`} size={18} />Registration Fee for Pair</li>
                        <li className='flex gap-2 items-center'><FaRegCalendar color={`${color}`} size={18} />Number of events each person can register</li>
                        <li className='flex gap-2 items-center'><FaRegCalendar color={`${color}`} size={18} />Protest Fee</li>
                        <li className='flex gap-2 items-center'><FaRegCalendar color={`${color}`} size={18} />Required Attachments</li>
