...
const SliderComponent = () => {
    ...
    const listDots = () => {
        return sliderImages.map((_,index) => (
            <div key={index} className={curIndex === index ? `w-3 h-3 bg-secondColor rounded-full hover:scale-110` : `w-2 h-2 bg-gray-300 rounded-full hover:scale-110`} />
        ))
    }
        ...
        <div className="relative w-full h-full flex flex-col gap-3 bg-transparent items-center">
            <div className="relative w-full h-full">
                ...
                {sliderImages.map((item) => (
                    <div
                        key={item.id}
                        className={`${activeSlide === item.id ? "block" : "hidden"
